...
def _process_github_detailed_payload(access_token, owner, repo_name, pull_number, head_sha, repo_full_name, pr_title,
    ...
    for file_path, file_data in structured_changes.items():
        ...
        reviews_for_file_list = get_openai_detailed_review_for_file(
            file_path,
            file_data,
            current_model,
            rag=rag_for_llm
        ...
        if reviews_for_file_list:  # reviews_for_file_list 是一个 Python 列表
            ...
            for review_item in reviews_for_file_list:
                if isinstance(review_item, dict) and used_sources:
                    review_item.setdefault("rag_sources", used_sources)
...
def _process_gitlab_detailed_payload(access_token, project_id_str, mr_iid, head_sha_payload, project_data, mr_attrs,
    ...
    rag_by_file = {}
    rag_sources_by_file = {}
    for fp, fd in structured_changes.items():
        try:
            rag_payload = rag_service.build_rag_context_for_file(
                "gitlab",
                project_id_str,
            ...
            if rag_payload.get("enabled") and rag_payload.get("context"):
                ...
                rag_sources_by_file[fp] = rag_payload.get("sources") or []
        except Exception:
            continue

    review_result_json = get_openai_code_review(structured_changes, rag_by_file=rag_by_file)
        ...
        for review in reviews:
            if isinstance(review, dict) and "file" in review:
                ...
                if file_path in structured_changes:
                    ...
                    used_sources = rag_sources_by_file.get(file_path) or []
                    if used_sources:
