...
def gitlab_webhook():
    ...
    future = executor.submit(run_with_app_context, app, routes_detailed._process_gitlab_detailed_payload,
        access_token=access_token, project_id_str=project_id_str, mr_iid=mr_iid, head_sha_payload=head_sha_payload,
        project_data=project_data, mr_attrs=mr_attrs, project_web_url=project_web_url, mr_title=mr_title, mr_url=mr_url,
...
def github_webhook():
    ...
    future = executor.submit(run_with_app_context, app, routes_detailed._process_github_detailed_payload,
        access_token=access_token, owner=owner, repo_name=repo_name, pull_number=pull_number, head_sha=head_sha,
        repo_full_name=repo_full_name, pr_title=pr_title, pr_html_url=pr_html_url, repo_web_url=repo_web_url,
        pr_source_branch=pr_source_branch, pr_target_branch=pr_target_branch, )
    future.add_done_callback(lambda f: handle_async_task_exception(f, logger_app_factory=logger))
...
def github_push_webhook():
    ...
    app = current_app._get_current_object()
    future = executor.submit(run_with_app_context, app, push_process._process_github_push_payload,
        access_token=access_token, owner=owner, repo_name=repo_name, repo_full_name=repo_full_name, ref=ref,
        audit_id=audit_id, before_sha=before_sha, after_sha=after_sha, created=created, default_branch=default_branch,
...
def gitlab_push_webhook():
    ...
    app = current_app._get_current_object()
    future = executor.submit(run_with_app_context, app, push_process._process_gitlab_push_payload,
        access_token=access_token, project_id_str=project_id_str, ref=ref, audit_id=audit_id, before_sha=before_sha,
        after_sha=after_sha, project_name=project_name, project_web_url=project_web_url, created=created,
