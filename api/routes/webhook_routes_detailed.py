...
def _post_no_issues_comment(vcs_type, comment_function, **comment_args_for_func):
    ...
    no_issues_review = {"file": overall_status_file, "severity": "INFO", "category": "General",
        "analysis": "AI Code Review 已完成，所有检查均已通过，无审查建议。", "suggestion": "Looks good!", "lines": {}
        # 确保这是一个通用的 PR/MR 评论
    }
...
def _process_github_detailed_payload(access_token, owner, repo_name, pull_number, head_sha, repo_full_name, pr_title,
    ...
    if not all_reviews_for_redis:  # 或者 total_comments_posted_successfully == 0
        _post_no_issues_comment(vcs_type='github', comment_function=add_github_pr_comment, owner=owner,
            repo_name=repo_name, pull_number=pull_number, access_token=access_token, head_sha=head_sha)
...
def github_webhook():
    ...
    future = executor.submit(_process_github_detailed_payload, access_token=access_token, owner=owner,
        repo_name=repo_name, pull_number=pull_number, head_sha=head_sha, repo_full_name=repo_full_name,
...
def gitlab_webhook():
    ...
    if event_type == "Push Hook":
        ...
        from api.routes.push_webhook_routes import gitlab_push_webhook
    ...
    # 调用提取出来的核心处理逻辑函数 (异步执行)
    future = executor.submit(_process_gitlab_detailed_payload, access_token=access_token, project_id_str=project_id_str,
        mr_iid=mr_iid, head_sha_payload=head_sha_payload, project_data=project_data, mr_attrs=mr_attrs,
