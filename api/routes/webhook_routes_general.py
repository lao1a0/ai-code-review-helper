...
from api.core_config import (github_repo_configs, gitlab_project_configs, app_configs, is_commit_processed,
                             mark_commit_as_processed, remove_processed_commit_entries_for_pr_mr)
...
from api.services.vcs_service import (get_github_pr_data_for_general_review, add_github_pr_general_comment,
...
def _process_github_general_payload(access_token, owner, repo_name, pull_number, pr_data, head_sha, repo_full_name,
    ...
    for file_item in file_data_list:
        ...
        if review_text_for_file and review_text_for_file.strip() and "未发现严重问题" not in review_text_for_file and "没有修改建议" not in review_text_for_file and "OpenAI client is not available" not in review_text_for_file and "Error serializing input data" not in review_text_for_file:
            ...
            review_wrapper_for_file = {"file": current_file_path, "lines": {"old": None, "new": None},
                "category": "general Review", "severity": "INFO", "analysis": review_text_for_file,
                "suggestion": "请参考上述分析。"}
    ...
    if aggregated_general_reviews_for_storage:
        ...
        _save_review_results_and_log(vcs_type='github_general', identifier=repo_full_name, pr_mr_id=str(pull_number),
            commit_sha=head_sha, review_json_string=review_json_string_for_storage, branch=pr_source_branch)
    else:
        ...
        _save_review_results_and_log(vcs_type='github_general', identifier=repo_full_name, pr_mr_id=str(pull_number),
            commit_sha=head_sha, review_json_string=json.dumps([]), branch=pr_source_branch  # Save empty list
        )
...
def github_webhook_general():
    ...
    if event_type == "push":
        ...
        from api.routes.push_webhook_routes import github_push_webhook
    ...
    future = executor.submit(_process_github_general_payload, access_token=access_token, owner=owner,
        repo_name=repo_name, pull_number=pull_number, pr_data=pr_data, head_sha=head_sha, repo_full_name=repo_full_name,
        pr_title=pr_title, pr_html_url=pr_html_url, repo_web_url=repo_web_url, pr_source_branch=pr_source_branch,
...
def _process_gitlab_general_payload(access_token, project_id_str, mr_iid, mr_attrs, final_position_info,
    ...
    if aggregated_general_reviews_for_storage:
        ...
        _save_review_results_and_log(vcs_type='gitlab_general', identifier=project_id_str, pr_mr_id=str(mr_iid),
            commit_sha=current_commit_sha_for_ops, review_json_string=review_json_string_for_storage,
            project_name_for_gitlab=project_name_from_payload, branch=mr_attrs.get('source_branch'))
    else:
        ...
        _save_review_results_and_log(vcs_type='gitlab_general', identifier=project_id_str, pr_mr_id=str(mr_iid),
            commit_sha=current_commit_sha_for_ops, review_json_string=json.dumps([]),  # Save empty list
            project_name_for_gitlab=project_name_from_payload, branch=mr_attrs.get('source_branch'))
...
def gitlab_webhook_general():
    ...
    # 调用提取出来的核心处理逻辑函数 (异步执行)
    future = executor.submit(_process_gitlab_general_payload, access_token=access_token, project_id_str=project_id_str,
        mr_iid=mr_iid, mr_attrs=mr_attrs, final_position_info=final_position_info, head_sha_payload=head_sha_payload,
        current_commit_sha_for_ops=current_commit_sha_for_ops, project_name_from_payload=project_name_from_payload,
