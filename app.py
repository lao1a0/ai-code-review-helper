...
from config.core_config import _load_env_file, gitlab_project_configs, remove_processed_commit_entries_for_pr_mr, \
...
def gitlab_webhook():
    ...
    future = executor.submit(hook._process_gitlab_detailed_payload, access_token=access_token,
...
def github_webhook():
    ...
    if event_type == "push":
        ...
        from hook.push_process import github_push_webhook
    ...
    # 调用提取出来的核心处理逻辑函数 (异步执行)
    future = executor.submit(hook._process_github_detailed_payload, access_token=access_token, owner=owner,
...
def gitlab_webhook_general():
    ...
    # 调用提取出来的核心处理逻辑函数 (异步执行)
    future = executor.submit(hook._process_gitlab_general_payload, access_token=access_token,
...
def github_webhook_general():
    ...
    # 调用提取出来的核心处理逻辑函数 (异步执行)
    future = executor.submit(hook._process_github_general_payload, access_token=access_token, owner=owner,
...
def github_push_webhook():
    ...
    audit_id = hook._make_push_audit_id(ref, after_sha)
    ...
    future = executor.submit(hook._process_github_push_payload, access_token=access_token, owner=owner,
...
def gitlab_push_webhook():
    ...
    future = executor.submit(hook._process_gitlab_push_payload, access_token=access_token,
