...
def build_agent():
    ...
    def github_list() -> str:
        ...
        repos = sorted(list((app_configs or {}).keys()))
        if not repos:
    ...
    def gitlab_list() -> str:
        ...
        projects = sorted(list((app_configs or {}).keys()),
    ...
    def github_add(repo_full_name: str, secret: str, token: str) -> str:
        ...
        app_configs[repo_full_name] = conf
    ...
    def github_delete(repo_full_name: str) -> str:
        ...
        repo_full_name = (repo_full_name or "").strip()
        if repo_full_name not in (app_configs or {}):
            return f"未找到 GitHub 仓库配置：{repo_full_name}"
        del app_configs[repo_full_name]
    ...
    def gitlab_delete(project_id: str) -> str:
        ...
        project_id = (project_id or "").strip()
        if project_id not in (app_configs or {}):
            return f"未找到 GitLab 项目配置：{project_id}"
        del app_configs[project_id]
