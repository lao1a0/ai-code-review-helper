...
class DatabaseService:
    ...
    # GitLab项目相关操作
    @staticmethod
    def create_gitlab_project(user_id: str, token: str, secret: Optional[str], 
                            project_id: str, url: str) -> GitLabProject:
        """创建GitLab项目"""
    ...
    # GitHub项目相关操作
    @staticmethod
    def create_github_project(user_id: str, token: str, secret: Optional[str],
                            repo: str, owner: str) -> GitHubProject:
        """创建GitHub项目"""
    ...
    # GitLab评审相关操作
    @staticmethod
    def create_gitlab_review(project_id: str, ai_name: str, review_content: str,
                           quality_score: Optional[float], commit_sha: str,
                           pr_mr_id: str, branch: Optional[str] = None) -> GitLabReview:
        """创建GitLab评审"""
        review = GitLabReview(
            project_id=project_id,
            ai_name=ai_name,
    ...
    def get_gitlab_reviews_by_project(project_id: str) -> List[GitLabReview]:
        """获取项目的所有GitLab评审"""
        return GitLabReview.query.filter_by(project_id=project_id).order_by(
            GitLabReview.created_at.desc()
        ).all()
    
    @staticmethod
    def get_gitlab_reviews_by_project_and_pr(project_id: str, pr_mr_id: str) -> List[GitLabReview]:
        """获取项目特定PR/MR的所有评审"""
        return GitLabReview.query.filter_by(
            project_id=project_id,
            pr_mr_id=pr_mr_id
        ).order_by(GitLabReview.created_at.desc()).all()
    
    # GitHub评审相关操作
    @staticmethod
    def create_github_review(project_id: str, ai_name: str, review_content: str,
                           quality_score: Optional[float], commit_sha: str,
                           pr_mr_id: str, branch: Optional[str] = None) -> GitHubReview:
        """创建GitHub评审"""
        review = GitHubReview(
            project_id=project_id,
            ai_name=ai_name,
    ...
    def get_github_reviews_by_project(project_id: str) -> List[GitHubReview]:
        """获取项目的所有GitHub评审"""
        return GitHubReview.query.filter_by(project_id=project_id).order_by(
    ...
    def get_github_reviews_by_project_and_pr(project_id: str, pr_mr_id: str) -> List[GitHubReview]:
        """获取项目特定PR/MR的所有评审"""
        return GitHubReview.query.filter_by(
            project_id=project_id,
            pr_mr_id=pr_mr_id
        ).order_by(GitHubReview.created_at.desc()).all()
