...
def delete_review_results_for_pr_mr(vcs_type: str, identifier: str, pr_mr_id: str):
    """Delete stored review results for one PR/MR."""
    try:
        ...
        logger.info("Deleted %s review result rows for %s %s #%s.",
                    deleted_count, vcs_type, identifier, pr_mr_id)
    except Exception as e:
        db.session.rollback()
        logger.error("Failed to delete review results: %s", e)
