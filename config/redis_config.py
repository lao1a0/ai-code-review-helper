...
def init_redis_client():
    ...
    if redis is None:
        err_msg = f"Redis library import failed"
        logger.critical(err_msg)
        raise ValueError(err_msg)
    ...
    try:
        ...
        for member_bytes in redis_client.sscan_iter(REDIS_PROCESSED_COMMITS_SET_KEY):
            ...
            if len(keys_to_remove_batch) >= batch_size:
                ...
                logger.debug(f"批处理：为 {vcs_type} {identifier} #{pr_mr_id} 从 Redis 中移除了 {removed_in_batch} 个条目。")
        ...
        if keys_to_remove_batch:
            removed_in_batch = redis_client.srem(REDIS_PROCESSED_COMMITS_SET_KEY, *keys_to_remove_batch)
            total_removed_count += removed_in_batch
            logger.debug(f"最后批处理：为 {vcs_type} {identifier} #{pr_mr_id} 从 Redis 中移除了 {removed_in_batch} 个条目。")

        if total_removed_count > 0:
            logger.info(f"为 {vcs_type} {identifier} #{pr_mr_id} 从 Redis 中总共移除了 {total_removed_count} 个已处理的 commit 条目。")
        else:
...
# --- 仓库/项目特定配置存储 (内存字典, 会在 Redis 模块中被填充) ---
# GitHub 仓库配置
