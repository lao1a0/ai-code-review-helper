...
from services import app_log, rag_call_graph, review_enrichment, settings_store, skill_registry
from services.vcs_service import get_github_pr_changes, get_gitlab_mr_changes

logger = logging.getLogger(__name__)

app = Blueprint("console_app", __name__)
...
def api_prompts_list():
    # prompt_loader already reads prompt/prompt_templates.yml; keep this lightweight: list keys only.
    import yaml
    from pathlib import Path

    path = Path("prompt") / "prompt_templates.yml"
    try:
...
def api_projects_list():
    ...
    for repo_full_name, conf in (core_config.github_repo_configs or {}).items():
    ...
    for project_id, conf in (core_config.gitlab_project_configs or {}).items():
        ...
        projects.append({
            ...
            "instance_url": (conf or {}).get("instance_url") or core_config.app_configs.get("GITLAB_INSTANCE_URL"),
            "has_token": bool((conf or {}).get("token")),
            "has_secret": bool((conf or {}).get("secret")),
            "settings": settings,
        })
...
@app.route("/api/projects/<path:project_key>/settings", methods=["GET"])
@require_admin_key
...
@app.route("/api/projects/<path:project_key>/settings", methods=["POST"])
@require_admin_key
...
def api_reviews_list():
    reviewed_items = core_config.get_all_reviewed_prs_mrs_keys() or []
    ...
    for item in reviewed_items:
        ...
        if include_stats:
            try:
                reviews_by_commit = core_config.get_review_results(vcs_type, identifier, pr_mr_id) or {}
...
@app.route("/api/reviews/<string:vcs_type>/<path:identifier>/<string:pr_mr_id>", methods=["GET"])
@require_admin_key
...
@app.route("/api/rag/call_graph", methods=["POST"])
@require_admin_key
def api_rag_call_graph():
    ...
    try:
        if base == "github":
            ...
            owner, repo = parsed
            cfg = core_config.github_repo_configs.get(identifier) or {}
            token = cfg.get("token")
            if not token:
            ...
            cfg = core_config.gitlab_project_configs.get(str(identifier)) or {}
            token = cfg.get("token")
            if not token:
